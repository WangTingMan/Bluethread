...
void l2cap_signaling::handle_config_response( std::vector<uint8_t> const& a_raw_hci )
{
    ...
    uint8_t ientifier = p_signaling[1];
    uint16_t signal_data_length = le_to_host16( p_signaling + 2 );
    uint16_t local_cid = le_to_host16( p_signaling + 4 );
    uint16_t flag = le_to_host16( p_signaling + 6 );
    uint16_t result = le_to_host16( p_signaling + 8 );

    uint8_t const* p_config = a_raw_hci.data() + m_sig_header.header_size() + 6;
    auto [options, unknown_types, is_truncated] = parse_channel_config( p_config, signal_data_length - 6 );
...
void l2cap_signaling::handle_disconnect_request( std::vector<uint8_t> const& a_raw_hci )
{
    uint8_t const* p_signaling = a_raw_hci.data() + m_sig_header.l2cap_header::header_size();
    uint8_t ientifier = p_signaling[1];
    uint16_t signal_data_length = le_to_host16( p_signaling + 2 );
    ...
    uint16_t local_cid = le_to_host16( p_signaling + 4 );
    uint16_t remote_cid = le_to_host16( p_signaling + 6 );

    std::shared_ptr<l2cap_disconnect_request> request;
    request = std::make_shared<l2cap_disconnect_request>();
    request->m_identifier = ientifier;
    request->m_destination_cid = local_cid;
    request->m_source_cid = remote_cid;
...
void l2cap_signaling::handle_echo_request( std::vector<uint8_t> const& a_raw_hci )
{
    uint8_t const* p_signaling = a_raw_hci.data() + m_sig_header.l2cap_header::header_size();
    uint8_t ientifier = p_signaling[1];
    ...
    m_sig_header.set_identifier( ientifier );
    m_sig_header.set_signaling_code( signaling_code::l2cap_echo_rsp );
    m_sig_header.set_sdu_length( signal_data_length );

    std::vector<uint8_t> buffer;
    buffer.resize( m_sig_header.header_size() + signal_data_length );

    m_sig_header.to_raw_buffer( buffer.data(), buffer.size() );
    memcpy( buffer.data() + m_sig_header.header_size(), p_signaling + 4, signal_data_length );
